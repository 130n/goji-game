import Phaser from 'phaser';
import { MONSTERS } from '../config/monsters.js';

const CATEGORY_COLORS = {
    balanced: 0x4488ff,
    tank: 0x44aa44,
    speed: 0xffaa22,
    bruiser: 0xff4444,
    charger: 0xaa44ff,
};

const CARD_WIDTH = 160;
const CARD_HEIGHT = 120;
const CARD_GAP_X = 20;
const CARD_GAP_Y = 16;
const GRID_COLS = 3;

// Selection states
const SEL_STATE = {
    P1_SELECTING: 'P1_SELECTING',
    P2_SELECTING: 'P2_SELECTING',
    READY: 'READY',
};

export class SelectScene extends Phaser.Scene {
    constructor() {
        super('Select');
    }

    init(data) {
        this.mode = data.mode || 'local';
        this.aiDifficulty = data.aiDifficulty || null;
    }

    create() {
        const centerX = 512;
        this.monsterList = Object.values(MONSTERS);

        // Background
        this.add.rectangle(centerX, 384, 1024, 768, 0x1a1a2e);

        // Title
        this.add.text(centerX, 40, 'SELECT YOUR MONSTER', {
            fontSize: '36px',
            fontFamily: 'monospace',
            color: '#ff6600',
            stroke: '#000000',
            strokeThickness: 4,
        }).setOrigin(0.5);

        // State
        this.selectionState = SEL_STATE.P1_SELECTING;
        this.p1CursorIndex = 0;
        this.p2CursorIndex = 0;
        this.p1Selection = null;
        this.p2Selection = null;

        // Build grid
        this.cards = [];
        this.createGrid(centerX);
        this.createSelectionInfo(centerX);
        this.createFightButton(centerX);
        this.createInstructions(centerX);
        this.setupInput();
        this.updateCursors();
    }

    createGrid(centerX) {
        const rows = Math.ceil(this.monsterList.length / GRID_COLS);
        const gridWidth = GRID_COLS * CARD_WIDTH + (GRID_COLS - 1) * CARD_GAP_X;
        const gridHeight = rows * CARD_HEIGHT + (rows - 1) * CARD_GAP_Y;
        const startX = centerX - gridWidth / 2 + CARD_WIDTH / 2;
        const startY = 120 + CARD_HEIGHT / 2;

        for (let i = 0; i < this.monsterList.length; i++) {
            const monster = this.monsterList[i];
            const col = i % GRID_COLS;
            const row = Math.floor(i / GRID_COLS);
            const x = startX + col * (CARD_WIDTH + CARD_GAP_X);
            const y = startY + row * (CARD_HEIGHT + CARD_GAP_Y);

            const card = this.createCard(x, y, monster, i);
            this.cards.push(card);
        }
    }

    createCard(x, y, monster, index) {
        const categoryColor = CATEGORY_COLORS[monster.category] || 0x888888;

        // Card background
        const bg = this.add.rectangle(x, y, CARD_WIDTH, CARD_HEIGHT, 0x222244)
            .setStrokeStyle(2, 0x444466);

        // Color stripe at top
        const stripe = this.add.rectangle(x, y - CARD_HEIGHT / 2 + 8, CARD_WIDTH - 8, 14, categoryColor)
            .setAlpha(0.8);

        // Monster name
        const nameText = this.add.text(x, y - 15, monster.name, {
            fontSize: '14px',
            fontFamily: 'monospace',
            color: '#ffffff',
            stroke: '#000000',
            strokeThickness: 2,
        }).setOrigin(0.5);

        // Stats
        const statsStr = `HP:${monster.hp} DMG:${monster.baseDamage} SPD:${monster.speedBonus}`;
        const statsText = this.add.text(x, y + 10, statsStr, {
            fontSize: '10px',
            fontFamily: 'monospace',
            color: '#aaaacc',
        }).setOrigin(0.5);

        // Category label
        const catText = this.add.text(x, y + 30, monster.category.toUpperCase(), {
            fontSize: '10px',
            fontFamily: 'monospace',
            color: Phaser.Display.Color.IntegerToColor(categoryColor).rgba,
        }).setOrigin(0.5);

        // P1 selection border (blue glow)
        const p1Border = this.add.rectangle(x, y, CARD_WIDTH + 6, CARD_HEIGHT + 6)
            .setStrokeStyle(3, 0x4488ff)
            .setFillStyle()
            .setAlpha(0);

        // P2 selection border (red glow)
        const p2Border = this.add.rectangle(x, y, CARD_WIDTH + 6, CARD_HEIGHT + 6)
            .setStrokeStyle(3, 0xff4444)
            .setFillStyle()
            .setAlpha(0);

        // P1 cursor (blue outline)
        const p1Cursor = this.add.rectangle(x, y, CARD_WIDTH + 10, CARD_HEIGHT + 10)
            .setStrokeStyle(2, 0x66ccff)
            .setFillStyle()
            .setAlpha(0);

        // P2 cursor (red outline)
        const p2Cursor = this.add.rectangle(x, y, CARD_WIDTH + 10, CARD_HEIGHT + 10)
            .setStrokeStyle(2, 0xff6666)
            .setFillStyle()
            .setAlpha(0);

        // Make card clickable
        bg.setInteractive({ useHandCursor: true });
        bg.on('pointerdown', () => {
            this.onCardClicked(index);
        });
        bg.on('pointerover', () => {
            this.onCardHover(index);
        });

        return {
            x, y, bg, stripe, nameText, statsText, catText,
            p1Border, p2Border, p1Cursor, p2Cursor, monster, index,
        };
    }

    createSelectionInfo(centerX) {
        this.selectionInfoText = this.add.text(centerX, 560, '', {
            fontSize: '22px',
            fontFamily: 'monospace',
            color: '#ffffff',
            stroke: '#000000',
            strokeThickness: 3,
        }).setOrigin(0.5);

        this.instructionText = this.add.text(centerX, 595, '', {
            fontSize: '16px',
            fontFamily: 'monospace',
            color: '#888899',
        }).setOrigin(0.5);

        this.updateSelectionInfo();
    }

    createFightButton(centerX) {
        this.fightButton = this.add.text(centerX, 650, 'FIGHT!', {
            fontSize: '36px',
            fontFamily: 'monospace',
            color: '#ff6600',
            stroke: '#000000',
            strokeThickness: 4,
        })
            .setOrigin(0.5)
            .setAlpha(0)
            .setInteractive({ useHandCursor: true });

        this.fightButton.on('pointerdown', () => {
            if (this.selectionState === SEL_STATE.READY) {
                this.startBattle();
            }
        });

        this.fightButton.on('pointerover', () => {
            if (this.selectionState === SEL_STATE.READY) {
                this.fightButton.setColor('#ffaa00');
            }
        });

        this.fightButton.on('pointerout', () => {
            this.fightButton.setColor('#ff6600');
        });
    }

    createInstructions(centerX) {
        this.controlHint = this.add.text(centerX, 740, '', {
            fontSize: '14px',
            fontFamily: 'monospace',
            color: '#555577',
        }).setOrigin(0.5);

        this.updateControlHint();
    }

    setupInput() {
        // P1 controls: WASD + Space
        this.input.keyboard.on('keydown-W', () => this.moveCursor('up'));
        this.input.keyboard.on('keydown-S', () => this.moveCursor('down'));
        this.input.keyboard.on('keydown-A', () => this.moveCursor('left'));
        this.input.keyboard.on('keydown-D', () => this.moveCursor('right'));
        this.input.keyboard.on('keydown-SPACE', () => this.confirmCurrent());

        // P2 controls: Arrow keys + Enter
        this.input.keyboard.on('keydown-UP', () => this.moveCursor('up'));
        this.input.keyboard.on('keydown-DOWN', () => this.moveCursor('down'));
        this.input.keyboard.on('keydown-LEFT', () => this.moveCursor('left'));
        this.input.keyboard.on('keydown-RIGHT', () => this.moveCursor('right'));
        this.input.keyboard.on('keydown-ENTER', () => this.confirmCurrent());

        // Escape to go back
        this.input.keyboard.on('keydown-ESC', () => this.goBack());
    }

    moveCursor(direction) {
        if (this.selectionState === SEL_STATE.READY) return;

        const isP1 = this.selectionState === SEL_STATE.P1_SELECTING;
        let idx = isP1 ? this.p1CursorIndex : this.p2CursorIndex;
        const total = this.monsterList.length;
        const col = idx % GRID_COLS;
        const row = Math.floor(idx / GRID_COLS);
        const rows = Math.ceil(total / GRID_COLS);

        switch (direction) {
            case 'up':
                if (row > 0) idx -= GRID_COLS;
                break;
            case 'down':
                if (row < rows - 1 && idx + GRID_COLS < total) idx += GRID_COLS;
                break;
            case 'left':
                if (col > 0) idx -= 1;
                break;
            case 'right':
                if (col < GRID_COLS - 1 && idx + 1 < total) idx += 1;
                break;
        }

        if (isP1) {
            this.p1CursorIndex = idx;
        } else {
            this.p2CursorIndex = idx;
        }

        this.updateCursors();
    }

    onCardHover(index) {
        if (this.selectionState === SEL_STATE.P1_SELECTING) {
            this.p1CursorIndex = index;
        } else if (this.selectionState === SEL_STATE.P2_SELECTING) {
            this.p2CursorIndex = index;
        }
        this.updateCursors();
    }

    onCardClicked(index) {
        if (this.selectionState === SEL_STATE.P1_SELECTING) {
            this.p1CursorIndex = index;
            this.confirmCurrent();
        } else if (this.selectionState === SEL_STATE.P2_SELECTING) {
            this.p2CursorIndex = index;
            this.confirmCurrent();
        }
    }

    confirmCurrent() {
        if (this.selectionState === SEL_STATE.P1_SELECTING) {
            this.p1Selection = this.monsterList[this.p1CursorIndex];
            this.showSelectionFlash(this.p1CursorIndex, 0x4488ff);

            if (this.mode === 'ai') {
                // AI auto-picks randomly
                const aiIdx = Phaser.Math.Between(0, this.monsterList.length - 1);
                this.p2CursorIndex = aiIdx;
                this.p2Selection = this.monsterList[aiIdx];

                this.time.delayedCall(300, () => {
                    this.showSelectionFlash(aiIdx, 0xff4444);
                    this.selectionState = SEL_STATE.READY;
                    this.updateCursors();
                    this.updateSelectionInfo();
                    this.updateControlHint();
                    this.showFightButton();
                });
            } else {
                this.selectionState = SEL_STATE.P2_SELECTING;
                this.updateCursors();
                this.updateSelectionInfo();
                this.updateControlHint();
            }
        } else if (this.selectionState === SEL_STATE.P2_SELECTING) {
            this.p2Selection = this.monsterList[this.p2CursorIndex];
            this.showSelectionFlash(this.p2CursorIndex, 0xff4444);
            this.selectionState = SEL_STATE.READY;
            this.updateCursors();
            this.updateSelectionInfo();
            this.updateControlHint();
            this.showFightButton();
        } else if (this.selectionState === SEL_STATE.READY) {
            this.startBattle();
        }
    }

    showSelectionFlash(cardIndex, color) {
        const card = this.cards[cardIndex];
        const flash = this.add.rectangle(card.x, card.y, CARD_WIDTH + 12, CARD_HEIGHT + 12)
            .setStrokeStyle(4, color)
            .setFillStyle(color, 0.15);

        this.tweens.add({
            targets: flash,
            alpha: 0,
            duration: 400,
            onComplete: () => flash.destroy(),
        });
    }

    showFightButton() {
        this.fightButton.setAlpha(0);
        this.tweens.add({
            targets: this.fightButton,
            alpha: 1,
            duration: 300,
            ease: 'Power2',
        });

        // Pulsing animation
        this.tweens.add({
            targets: this.fightButton,
            scale: 1.1,
            duration: 500,
            yoyo: true,
            repeat: -1,
            ease: 'Sine.easeInOut',
        });
    }

    updateCursors() {
        for (let i = 0; i < this.cards.length; i++) {
            const card = this.cards[i];

            // P1 cursor
            const showP1Cursor = this.selectionState === SEL_STATE.P1_SELECTING && i === this.p1CursorIndex;
            card.p1Cursor.setAlpha(showP1Cursor ? 1 : 0);

            // P2 cursor
            const showP2Cursor = this.selectionState === SEL_STATE.P2_SELECTING && i === this.p2CursorIndex;
            card.p2Cursor.setAlpha(showP2Cursor ? 1 : 0);

            // P1 selection border (stays after confirm)
            const isP1Selected = this.p1Selection && this.p1Selection.id === card.monster.id;
            card.p1Border.setAlpha(isP1Selected ? 1 : 0);

            // P2 selection border
            const isP2Selected = this.p2Selection && this.p2Selection.id === card.monster.id;
            card.p2Border.setAlpha(isP2Selected ? 1 : 0);
        }
    }

    updateSelectionInfo() {
        const p1Name = this.p1Selection ? this.p1Selection.name : '???';
        const p2Name = this.p2Selection ? this.p2Selection.name : '???';
        this.selectionInfoText.setText(`P1: ${p1Name}  vs  P2: ${p2Name}`);

        if (this.selectionState === SEL_STATE.P1_SELECTING) {
            this.instructionText.setText('PLAYER 1 - Choose your monster');
        } else if (this.selectionState === SEL_STATE.P2_SELECTING) {
            this.instructionText.setText('PLAYER 2 - Choose your monster');
        } else {
            this.instructionText.setText('Press ENTER or click FIGHT to begin!');
        }
    }

    updateControlHint() {
        if (this.selectionState === SEL_STATE.P1_SELECTING) {
            this.controlHint.setText('WASD = Move   SPACE = Select   ESC = Back');
        } else if (this.selectionState === SEL_STATE.P2_SELECTING) {
            this.controlHint.setText('Arrows = Move   ENTER = Select   ESC = Back');
        } else {
            this.controlHint.setText('ENTER = Fight!   ESC = Back');
        }
    }

    goBack() {
        if (this.selectionState === SEL_STATE.P2_SELECTING) {
            // Go back to P1 selection
            this.p1Selection = null;
            this.selectionState = SEL_STATE.P1_SELECTING;
            this.updateCursors();
            this.updateSelectionInfo();
            this.updateControlHint();
        } else if (this.selectionState === SEL_STATE.READY) {
            // Go back to P2 selection (or P1 in AI mode)
            this.p2Selection = null;
            this.fightButton.setAlpha(0);
            this.tweens.killTweensOf(this.fightButton);
            this.fightButton.setScale(1);

            if (this.mode === 'ai') {
                this.p1Selection = null;
                this.selectionState = SEL_STATE.P1_SELECTING;
            } else {
                this.selectionState = SEL_STATE.P2_SELECTING;
            }

            this.updateCursors();
            this.updateSelectionInfo();
            this.updateControlHint();
        } else {
            // Back to menu
            this.scene.start('Menu');
        }
    }

    startBattle() {
        this.scene.start('Battle', {
            player1: this.p1Selection,
            player2: this.p2Selection,
            mode: this.mode,
            aiDifficulty: this.aiDifficulty,
        });
    }
}
